...
Literature: Field (4th ed, 2013) Chapter. 2, Simmons, Nelson, & Simonsohn (2011)
...
**(1G)**
See the smokers-on-the-treadmill-example from Chapter 1 (self-test question 5), with outcomes of time (in seconds) taken for 21 heavy smokers to fall off a treadmill at the fastest setting. For this exercise we only take into account the first four observations: 18, 16, 18, 24.
...
To compute the margin of error for the mean, we need to find the appropriate value of $t$. To do so, we calulate the degrees of freedom $df=N-1$. With 11 data points the degrees of freedom is $df=10$. For a 95% confidence interval, we look up the value in the column labbeled ``Two-Tailed Test'', '0.05' in the table of the critical values of the $t$-distribution (see the Appendix of the Field book). The corresponding value is $t=2.23$

The confidence intervals ($CI$) are therefore:

* Lower Bounder of the $CI = \overline{X}- (2.23 \times SE) = 96.64-(2.23\times 18.47) = 55.63$
* Upper Bounder of the $CI = \overline{X}+ (2.23 \times SE) = 96.64+(2.23\times 18.47) = 137.65$

**Answer  1B**

The new standard error is
$$\sigma_{\overline{X}}=\frac{s}{\sqrt{N}}=\frac{61.27}{\sqrt{56}}=8.19$$
The appropriate value for $t$ with $df = N-1= 55$ cannot be read directly from the $t$-table in Field. You can use the conservative approximation by choosing $t(50) = 2.01$

The 95% CI for $N =56$ is:

* Lower Bounder of the $CI = \overline{X}- (2.01 \times SE) = 96.64-(2.01\times 8.19) = 80.18$
* Upper Bounder of the $CI = \overline{X}+ (2.01 \times SE) = 96.64+(2.01\times 8.19) = 113.10$
