...
Literature: Field (2013) Chapter 15: repeated measures ANOVA with one or more between-subjects factors
...
## Repeated measures ANOVA mixed design, follow-up: contrasts
...
**(1D)**
Write a “short report” (see Field p. 617-619) based on the information above. Also compute the effect size for the contrast (see Field p. 616-617):

$$r = \sqrt{\frac{F_\text{effect}}{F_\text{effect} + df_\text{effect}}}$$
...
**(2G)**
What strategy does Field (2013) recommend for interpreting/reporting the three-way interaction in this example?
...
Group: $\mu(\text{non-fearfull}) =\mu(\text{fearful})$

Stimulus: $\mu(\text{spider}) =\mu(\text{neutral})$

Group $\times$ Stimulus:

* $\mu(\text{non-fearfull, spider}) - \mu(\text{non-fearfull, neutral}) =  \mu(\text{fearful, spider})-\mu(\text{fearful, neutral})$ or
* $\mu(\text{non-fearfull, spider}) -  \mu(\text{fearful, spider}) = \mu(\text{non-fearfull, neutral}) -\mu(\text{fearful, neutral})$
...
Note: $df_\text{model}=df_\text{effect}=1$

There are no differences between both groups (fearful, non-fearful) in terms of the average number of changes that were noticed by participants, $F(1,48) < 1, r = .01$. There was a strong, significant main effect of stimulus, $F(1,48) = 52.23, p < .001, r = .72$; on average the number of changes that were noticed by participants was larger when the stimulus was a spider.
...
Three-way mixed ANOVA (see also Field, p. 423)
...
factor - 1.
* The number of contrast-tests for an interaction effect between factors equals the $(\text{number of levels for factor1} - 1) \times (\text{number of levels for factor2} -1)$\newline $=(nlv_\text{factor 1}-1) \times (nlv_\text{factor 2}-1)$
* Or for a three-way interaction: $(nlv_\text{factor 1}-1) \times (nlv_\text{factor 2}-1) \times (nlv_\text{factor 3}-1)$.
...
Inspect the interaction between the two ws-factors for both groups separately (see Field p. 527). Thus make an interaction plot of time vs. type of sum for the PC group and the traditional group.
