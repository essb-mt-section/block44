...
Literature: Field (2013) Chapter. 8
...
# Chapter 8 (Field, 2013)

## Regression

**(1A)**
The significance of regression coefficient in a regression model is tested with a t-test. How is the $t$ in SPSS output 8.3 below (Field, 2013, p. 319) calculated? Use the values in the table to see if you can get the same values as SPSS.
...
**(1C)**
See the SPSS output 8.7 (Field 2013. p. 339) below for Model 2, where record sales are predicted from advertising budget, number of plays on the radio per week, and attractiveness of the band:
...
**(1E)**
Think back to what the confidence interval of the mean represented (Field, 2013, section 2.5.2). Can you describe what the confidence intervals for the regression coefficients $b$ represent? What can you conclude about the accuracy of the estimated $b$’s?
...
**(6C)**
Determine with Figure 8.9 (Field, 2013, p. 314) what the minimum sample size must be, to achieve a power of .80, for a regression model test with **5 predictors and a medium expected effect size**.
...
Using the values from SPSS output 8.3 (Field, 2013, p. 319) to calculate $t$ for the constant ($b_0$):
...
See the SPSS output 8.7 (Field 2013. p. 339) below for Model 2, where record sales are predicted from advertising
...
If, for instance, two predictors are highly correlated, then omit one on theoretical grounds. If, for instance, a sets of predictors is highly inter-correlated, then consider combining the scores on these predictors into one or two “super” variables. A data analysis technique that can be used for this purpose is factor analysis (see $4^\text{th}$ edition Field (2013) chapter 17).
